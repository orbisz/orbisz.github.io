import{_ as l,c as t,a as i,b as n,d as s,e as r,w as d,r as c,o as p}from"./app-BnTl11ii.js";const u={};function o(v,e){const a=c("scope");return p(),t("div",null,[e[6]||(e[6]=i('<h3 id="思考" tabindex="-1"><a class="header-anchor" href="#思考"><span>思考</span></a></h3><h4 id="为什么使用redis作为注册中心而不是-nacos-consul-zookeeper" tabindex="-1"><a class="header-anchor" href="#为什么使用redis作为注册中心而不是-nacos-consul-zookeeper"><span>为什么使用Redis作为注册中心而不是 Nacos/Consul/Zookeeper？</span></a></h4><p>本项目使用轻量级的动态配置中心组件（Dynamic Config Center），主要用于管理应用配置、功能开关等简单配置项。 从代码实现来看，项目使用了Redisson客户端与Redis进行交互，实现了配置的动态获取、注入和更新功能。</p><p>相比Nacos/Consul/Zookeeper，Redis部署和维护更加简单，资源占用更少，特别适合小型项目或对配置中心功能需求相对简单的场景。 作为轻量级的组件，引入Nacos/Consul/Zookeeper会引入额外的复杂度，在微服务应用中引入该组件时可以考虑根据实际需求景使用其他的方式作为注册中心</p><p>项目利用Redis的发布/订阅功能实现了配置变更的实时通知机制，当配置更新时，可以立即通知到所有相关服务。</p><h4 id="基于-spi-机制的组件框架中-如何定义组件接口和实现类-业务系统如何-按需引入-组件-如只引入限流组件-不引入动态配置-组件版本冲突如何解决" tabindex="-1"><a class="header-anchor" href="#基于-spi-机制的组件框架中-如何定义组件接口和实现类-业务系统如何-按需引入-组件-如只引入限流组件-不引入动态配置-组件版本冲突如何解决"><span>基于 SPI 机制的组件框架中，如何定义组件接口和实现类？业务系统如何 “按需引入” 组件（如只引入限流组件，不引入动态配置）？组件版本冲突如何解决？</span></a></h4><p>在项目中，组件接口和实现类采用了清晰的分层设计模式</p><ul><li>使用Java标准接口定义组件能力边界，如 <code>StrategyHandler&lt;T, D, R&gt;</code> 和 <code>IDynamicConfigCenterService</code> 广泛使用泛型（T、D、R）提高接口灵活性和复用性；接口方法设计简洁明确，聚焦核心功能。</li><li>实现类实现对应接口，并通过 @Component 、 @Service 等Spring注解注册为Bean； 例如 <code>DynamicConfigCenterService</code> 实现了 <code>IDynamicConfigCenterService</code> 接口；采用继承抽象类（如 AbstractXxxSupport ）的方式复用通用逻辑。</li><li>利用Spring Boot的 spring.factories 机制实现组件的自动装配，如限流组件在 spring.factories 中配置了 RateLimiterAutoConfig。</li></ul><p>项目采用Maven模块化结构和Spring Boot自动配置机制实现按需引入;</p>',9)),n("ul",null,[e[3]||(e[3]=n("li",null,"每个功能组件独立成模块，如 orbisz-wrench-starter-rate-limiter （限流）、 orbisz-wrench-starter-dynamic-config-center （动态配置中心）;各模块间通过Maven依赖关系管理。",-1)),e[4]||(e[4]=n("li",null,"业务系统（如测试项目 orbisz-wrench-test ）在其pom.xml中只引入所需组件依赖；例如如需限流功能，只需添加限流组件的依赖，不需要引入动态配置中心。",-1)),e[5]||(e[5]=n("li",null,"利用 @ConditionalOnMissingBean 等注解实现条件装配，组件通过 spring.factories 声明自动配置类，但只有在被引入时才会激活。",-1)),n("li",null,[e[1]||(e[1]=s("组件间依赖通过 ")),r(a,null,{default:d(()=>e[0]||(e[0]=[s("provided")])),_:1,__:[0]}),e[2]||(e[2]=s(" 设置为可选；如限流组件中对动态配置中心的依赖。"))])]),e[7]||(e[7]=i(`<div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">&lt;dependency&gt;</span>
...
<span class="line">&lt;/dependency&gt;</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ul><li>利用Spring Boot的依赖管理能力，继承Spring Boot的父POM,享受Spring Boot提供的依赖版本仲裁功能</li></ul><h4 id="自定义-dccvalue注解以及beanpostprocessor-拦截机制的实现。beanpostprocessor-的拦截时机是什么-你实现的-dccvalue-注解热更新机制里-如何保证配置更新的原子性和可见性" tabindex="-1"><a class="header-anchor" href="#自定义-dccvalue注解以及beanpostprocessor-拦截机制的实现。beanpostprocessor-的拦截时机是什么-你实现的-dccvalue-注解热更新机制里-如何保证配置更新的原子性和可见性"><span>自定义@DCCValue注解以及BeanPostProcessor 拦截机制的实现。BeanPostProcessor 的拦截时机是什么？你实现的 @DCCValue 注解热更新机制里，如何保证配置更新的原子性和可见性？</span></a></h4><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">@Retention(RetentionPolicy.RUNTIME)</span>
...
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ol><li>基于数据库的计数器： <ul><li>使用数据库行锁实现原子计数</li><li>适合低并发场景，性能较差</li></ul></li><li>基于消息队列： <ul><li>使用 RabbitMQ、Kafka 等消息队列控制流量</li><li>天然支持削峰填谷</li></ul></li><li>基于分布式锁： <ul><li>使用 Zookeeper、etcd 等实现分布式锁</li><li>保证全局唯一性，但性能开销大</li></ul></li><li>网关层限流： <ul><li>在 API 网关（如 Kong、Zuul、Gateway）层实现</li><li>统一管理，配置灵活</li></ul></li></ol><h4 id="动态黑名单的失效策略-定时扫库-vs-redis过期" tabindex="-1"><a class="header-anchor" href="#动态黑名单的失效策略-定时扫库-vs-redis过期"><span>动态黑名单的失效策略（定时扫库 vs Redis过期）？</span></a></h4><p><strong>项目使用 Guava Cache 实现黑名单管理</strong></p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">// 个人限频黑名单24h - 分布式业务场景，可以记录到 Redis 中</span>
...
<span class="line">    }</span>
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="黑名单拦截的实现和降级回调-降级回调如何设计才能避免雪崩效应" tabindex="-1"><a class="header-anchor" href="#黑名单拦截的实现和降级回调-降级回调如何设计才能避免雪崩效应"><span>黑名单拦截的实现和降级回调，降级回调如何设计才能避免雪崩效应？</span></a></h4><p>黑名单拦截</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">// 黑名单缓存，24小时过期</span>
<span class="line">private final Cache&lt;String, Long&gt; blacklist = CacheBuilder.newBuilder()</span>
<span class="line">.expireAfterWrite(24, TimeUnit.HOURS)</span>
<span class="line">.build();</span>
